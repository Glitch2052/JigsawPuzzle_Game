...
namespace PolyAndCode.UI
{
    ...
    public class VerticalRecyclingSystem : RecyclingSystem
    {
        ...
        public override IEnumerator InitCoroutine(Action onInitialized)
        {
            ...
            yield return null;
            SetRecyclingBounds();
            Content.TryGetComponent(out layoutData);
            ...
            float contentYSize = noOfRows * _cellHeight + ((noOfRows - 1) * layoutData.spacing.y) + layoutData.topPadding + layoutData.bottomPadding;
        ...
        /// <summary>
        /// Resets cell pool if present or created a new one if empty
        /// </summary>
        ...
        private void CreateCellPool()
        {
            ...
            int poolSize = 0;
            float posX = layoutData.leftPadding;
            float posY = - layoutData.topPadding;
            ...
            _cellWidth = layoutData.cellSize.x;
            _cellHeight = layoutData.cellSize.y;
            ...
            while ((poolSize < minPoolSize || currentPoolCoverage < requriedCoverage) && poolSize < DataSource.GetItemCount())
            {
                ...
                RectTransform item = Object.Instantiate(PrototypeCell, Content, false);
                item.name = "Cell";
                ...
                if (IsGrid)
                {
                    posX = _bottomMostCellColoumn * (_cellWidth + layoutData.spacing.x) + layoutData.leftPadding;
                    item.anchoredPosition = new Vector2(posX, posY);
                    if (++_bottomMostCellColoumn >= _coloumns)
                    {
                        _bottomMostCellColoumn = 0;
                        posY -= (_cellHeight + layoutData.spacing.y);
                ...
                else
                {
                    ...
                    posY = item.anchoredPosition.y - item.rect.height - layoutData.spacing.y;
                    currentPoolCoverage += item.rect.height;
                }

                //Setting data for Cell
                item.TryGetComponent(out ICell cell);
                _cachedCells.Add(cell);
                DataSource.InitCell(_cachedCells[^1]);
        ...
        private Vector2 RecycleTopToBottom()
        {
            ...
            while (_cellPool[topMostCellIndex].MinY() > _recyclableViewBounds.max.y && currentItemCount < DataSource.GetItemCount())
            {
                if (IsGrid)
                {
                    if (++_bottomMostCellColoumn >= _coloumns)
                    {
                        n++;
                        _bottomMostCellColoumn = 0;
                        posY = _cellPool[bottomMostCellIndex].anchoredPosition.y - _cellHeight - layoutData.spacing.y;
                        additionalRows++;
                    }
                ...
                {
                    //Move top cell to bottom
                    posY = _cellPool[bottomMostCellIndex].anchoredPosition.y - _cellPool[bottomMostCellIndex].sizeDelta.y - layoutData.spacing.y;
            ...
            if (IsGrid)
            {
                Content.sizeDelta += Vector2.up * (additionalRows * (_cellHeight + layoutData.spacing.y));
            ...
            _cellPool.ForEach((RectTransform cell) => cell.anchoredPosition += Vector2.up * (n * (_cellPool[topMostCellIndex].sizeDelta.y + layoutData.spacing.y)));
            Content.anchoredPosition -= Vector2.up * (n * (_cellPool[topMostCellIndex].sizeDelta.y + layoutData.spacing.y));
            _recycling = false;
            return -new Vector2(0, n * (_cellPool[topMostCellIndex].sizeDelta.y + layoutData.spacing.y));
        }
            ...
            while (_cellPool[bottomMostCellIndex].MaxY() < _recyclableViewBounds.min.y && currentItemCount > _cellPool.Count)
            {
                if (IsGrid)
                {
                    if (--_topMostCellColoumn < 0)
                    {
                        ...
                        posY = _cellPool[topMostCellIndex].anchoredPosition.y + _cellHeight + layoutData.spacing.y;
                        additionalRows++;
                    }

                    //Move bottom cell to top
                    posX = _topMostCellColoumn * (_cellWidth + layoutData.spacing.x) + layoutData.leftPadding;;
                ...
                {
                    //Move bottom cell to top
                    posY = _cellPool[topMostCellIndex].anchoredPosition.y + _cellPool[topMostCellIndex].sizeDelta.y + layoutData.spacing.y;
